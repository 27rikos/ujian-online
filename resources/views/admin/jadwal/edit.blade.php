...
<div class="pc-content">
    ...
    <div class="row">
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('jadwal.update',$find->id) }}" method="post">
                        @method('put')
                        @csrf
                        <div class="form-group">
                            <label for="tanggal" class="mb-2">Tanggal</label>
                            <input type="date" class="form-control" name="tanggal" value="{{ $find->tanggal }}">
                        </div>
                        <div class="form-group">
                           ...
                           <select name="matakuliah" id="matakuliah" class="form-control">
                            ...
                            @foreach ($matakuliah as $item)
                                <option value="{{ $item->matakuliah }}" {{ $find->matakuliah==$item->matakuliah ? 'selected':'' }}>{{ $item->matakuliah }}</option>
                            @endforeach
                           </select>
                        </div>
                        <div class="form-group">
                           ...
                           <div class="row">
                            <div class="col-md-5">
                                <input type="time" class="form-control" name="start" value="{{ $find->start }}">
                            </div>
                            <div class="col-md-2 text-center mt-2">Sampai</div>
                            <div class="col-md-5">
                                <input type="time" class="form-control" name="end" value="{{ $find->end }}">
                            </div>
                           </div>
                        </div>
                        <div class="form-group">
                            <label for="sks" class="mb-2">SKS</label>
                            <input type="text" class="form-control" name="sks" value="{{ $find->sks }}">
                        </div>
                        <div class="form-group">
